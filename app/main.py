...
def analyze_sentiment_and_phrases(text: str) -> dict:
    ...
    key_phrases_doc = key_phrases_response[0]

    return {
        "sentiment": sentiment_doc.sentiment,
        "confidence_scores": {
            "positive": sentiment_doc.confidence_scores.positive,
            "neutral": sentiment_doc.confidence_scores.neutral,
            "negative": sentiment_doc.confidence_scores.negative,
        },
        "key_phrases": key_phrases_doc.key_phrases,
    }
...
def detect_and_translate(text: str) -> dict:
    ...
    lang_doc = lang_response[0]
    detected_language = lang_doc.primary_language.iso6391_name
    confidence = lang_doc.primary_language.confidence_score
    ...
    try:
        ...
        confidence = next(
            (i.confidence_score for i in prediction.intents
             if i.category == raw_intent), 0.0
        )
        entities = [
            {"entity": e.text, "category": e.category, "text": e.text}
